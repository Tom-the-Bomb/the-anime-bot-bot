...
class logging(commands.Cog):
    ...
    async def logging(self, ctx):
        ...
        settings = [
            f"{i} - {str(v).replace(logging_settings['webhook'], 'secrect webhook')}" for i, v in logging_settings.items()]
        settings = "\n".join(settings)
    ...
    async def on_guild_channel_create(self, channel):
        if channel.guild.id not in self.bot.logging_cache.keys():
            return
        embed = discord.Embed(color=self.bot.color, title="New channel created",
    ...
    async def on_guild_channel_update(self, before, after):
        if after.guild.id not in self.bot.logging_cache.keys():
            return
        embed = discord.Embed(
            color=self.bot.color, title="Channel Updated", timestamp=after.created_at)
    ...
    async def on_guild_channel_delete(self, channel):
        if channel.guild.id not in self.bot.logging_cache.keys():
            return
        embed = discord.Embed(color=self.bot.color, title="Channel Deleted",
                              description=f"**channel name:** {channel.name}\n**Category:** {channel.category}", timestamp=channel.created_at)
        embed.set_footer(text=f"Channel ID: {channel.id}")
    ...
    async def on_guild_role_create(self, role):
        if role.guild.id not in self.bot.logging_cache.keys():
            return
        embed = discord.Embed(color=self.bot.color, title="Role Created",
    ...
    async def on_guild_role_update(self, before, after):
        if before.color != after.color:
            embed = discord.Embed(color=self.bot.color, title="Role Updated",
        ...
        if before.permissions != after.permissions:
            changed_permission = [(i, v) for i, v in dict(
                after).items() if dict(before)[i] != v]
            to_display = []
            ...
            to_display = "\n".join(to_display)
            embed = discord.Embed(color=self.bot.color, title="Role Updated",
                                  description=f"**New Permissions:** {to_display}", timestamp=datetime.datetime.utcnow())
            await self.send_webhook(after.guild.id, embed, "role_update")
    ...
    async def on_guild_role_delete(self, role):
        if role.guild.id not in self.bot.logging_cache.keys():
            return
        embed = discord.Embed(color=self.bot.color, title="Role Deleted",
                              description=f"**Role name:** {role.name}\n**Color:** {str(channel.color)}", timestamp=channel.created_at)
        embed.set_footer(text=f"Channel ID: {channel.id}")
        await self.send_webhook(role.guild.id, embed, "role_delete")
    ...
    async def on_guild_update(self, before, after):
        if before.afk_channel != after.afk_channel:
            embed = discord.Embed(color=self.bot.color, title="Guild Updated",
        ...
        embed = discord.Embed(color=self.bot.color, title="Guild Updated",
                              description=f"Something updated but I can't trace what updated", timestamp=datetime.datetime.utcnow())
    ...
    async def on_guild_emojis_update(self, guild, before, after):
        if len(before) < len(after):
            ...
            embed = discord.Embed(color=self.bot.color, title="New Emojis Added",
                                  description=f"{', '.join(new_emojis)}", timestamp=datetime.datetime.utcnow())
            await self.send_webhook(after.guild.id, embed, "emojis_update")
        else:
            c = [str(i) for i in before if i not in after]
            embed = discord.Embed(color=self.bot.color, title="Emojis Removed",
                                  description=f"{', '.join(removed_emojis)}", timestamp=datetime.datetime.utcnow())
            await self.send_webhook(after.guild.id, embed, "emojis_update")

    @commands.Cog.listener()
    async def on_member_update(self, before, after):
        if before.nick != after.nick:
            embed = discord.Embed(color=self.bot.color, title="Nickname Changed",
        ...
        if before.roles != after.roles:
            if len(before.roles) < len(after.roles):
                ...
                embed = discord.Embed(color=self.bot.color, title="Role Added",
                ...
                embed = discord.Embed(color=self.bot.color, title="Role Removed",
    ...
    async def on_member_ban(self, guild, user):
        embed = discord.Embed(color=self.bot.color, title="Member Banned",
                              description=f"User: {str(user)} {user.mention} ({user.id})", timestamp=datetime.datetime.utcnow())
        embed.set_footer(text=f"User ID: {user.id}")
        await self.send_webhook(guild.id, embed, "member_ban")

    @commands.Cog.listener()
    async def on_member_unban(self, guild, user):
        embed = discord.Embed(color=self.bot.color, title="Member Unbanned",
                              description=f"User: {str(user)} {user.mention} ({user.id})", timestamp=datetime.datetime.utcnow())
        embed.set_footer(text=f"User ID: {user.id}")
    ...
    async def on_invite_create(self, invite):
        embed = discord.Embed(color=self.bot.color, title="Invite Created",
    ...
    async def on_invite_delete(self, invite):
        embed = discord.Embed(color=self.bot.color, title="Invite Deleted",
    ...
    async def on_member_join(self, member):
        embed = discord.Embed(color=self.bot.color, title="Member joined",
                              description=f"{member.mention} just joined. We now have {member.guild.member_count} members. Account Created at: {humanize.precisedelta(member.created_at)} ago", timestamp=datetime.datetime.now())
        embed.set_footer(text=f"ID: {member.id}")
        await self.send_webhook(member.guild.id, embed, "member_join")

    @commands.Cog.listener()
    async def on_member_remove(self, member):
        embed = discord.Embed(color=self.bot.color, title="Member left",
                              description=f"{member.mention} just left. We now have {member.guild.member_count} members. Account Created at: {humanize.precisedelta(member.created_at)} ago", timestamp=datetime.datetime.now())
        embed.set_footer(text=f"ID: {member.id}")
    ...
    async def on_voice_state_update(self, member, before, after):
        if not before.channel:
            embed = discord.Embed(color=self.bot.color, title="Joined voice channel",
        ...
        if not after.channel:
            embed = discord.Embed(color=self.bot.color, title="Left voice channel",
                                  description=f"{member.display_name} left voice channel {before.channel.mention}")
            embed.set_footer(text=f"User ID: {member.id}")
            return await self.send_webhook(member.guild.id, embed, "voice_channel_change")
        if before.channel != after.channel:
            embed = discord.Embed(color=self.bot.color, title="Changed voice channel",
    ...
    async def on_message_edit(self, before, after):
        ...
        embed = discord.Embed(
            color=self.bot.color, title="Message Edited", timestamp=datetime.datetime.utcnow())
        embed.add_field(name="Before", value=f"**Content:** {before_content}")
        embed.add_field(name="After", value=f"**Content:** {after_content}")
        embed.set_author(name=str(after.author), icon_url=str(
            after.author.avatar_url_as(static_format="png")))
    ...
    async def on_raw_bulk_message_delete(self, payload):
        ...
        embed = discord.Embed(
            color=self.bot.color, title="Bulk messages deleted", description=f"Message Ids: \n{ids}")
    ...
    async def on_raw_message_delete(self, payload):
        ...
        if message:
            ...
            embed = discord.Embed(color=self.bot.color, title="Message Deleted",
                                  description=f"**Content:** {content}", timestamp=datetime.datetime.utcnow())
            embed.set_author(name=str(message.author), icon_url=str(
            ...
            embed = discord.Embed(color=self.bot.color, title="Message Deleted",
                                  description=f"The message is too old I can't find the content", timestamp=datetime.datetime.utcnow())
            await self.send_webhook(payload.guild_id, embed, "message_delete")
