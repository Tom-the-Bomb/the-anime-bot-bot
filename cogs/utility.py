...
class Transformer(ast.NodeTransformer):
    ...
    ALLOWED_NODE_TYPES = set([
        'Expression', 'Int', 'Float', 'Call', 'UnaryOp', 'Not', 'Invert',
        'UAdd', 'USub', 'Compare', 'Eq', 'Num', 'Constant', 'BinOp', 'Add',
        'Div', 'FloorDiv', 'Sub', 'Mult', 'BitXor', 'op', 'Pow', 'Mod'
    ])
...
class utility(commands.Cog):
    ...
    """
        ...
        embed = discord.Embed(color=self.bot.color, description=f"[webp]({str(member1.avatar_url_as(format='webp'))}) [jpeg]({str(member1.avatar_url_as(format='jpeg'))}) [png]({str(member1.avatar_url_as(format='png'))})" if not ctx.author.is_avatar_animated() else f"[webp]({str(member1.avatar_url_as(format='webp')))}) [jpeg]({str(member1.avatar_url_as(format='jpeg'))}) [png]({str(member1.avatar_url_as(format='png'))}) [gif]({str(member1.avatar_url_as(format='gif'))})")
