...
class pictures(commands.Cog):
    ...
    async def cdn_(self, url):
        async with ratelimiter.RateLimiter(max_calls=3, period=7):
            async with self.bot.session.get(url) as resp:
                ...
                async with self.bot.session.post("https://idevision.net/api/cdn", headers={"Authorization": config.idevision}, data=BytesIO(await resp.read())h) as resp:
    ...
    async def amiajoke(
            self,
            ctx,
            thing: typing.Optional[typing.Union[discord.Member, discord.User,
                                                discord.PartialEmoji,
                                                discord.Emoji, str]],
            level: float = 0.3):
    ...
    async def threshold(self,
                        ctx,
                        thing: typing.Union[discord.Member, discord.User,
                                            discord.PartialEmoji,
                                            discord.Emoji, str] = None):
        async with ctx.channel.typing():
            url = await self.get_url(ctx, thing)
