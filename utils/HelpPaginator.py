...
class Pages:
    ...
    async def show_page(self, page, *, first=False):
        ...
        p = [f"{index}. {entry}" for index, entry in enumerate(entries, 1 + ((page - 1) * self.per_page))]
    ...
    def react_check(self, payload):
        if payload.user_id is None or payload.user_id != self.author.id:
            return False
    ...
    async def paginate(self):
        ...
        while self.paginating:
            try:
                ...
                done, pending = await asyncio.wait(tasks, timeout=7200, return_when=asyncio.FIRST_COMPLETED)
                for task in pending:
                    task.cancel()
                if len(done) == 0:
...
class HelpPaginator(Pages):
    ...
    async def from_command(cls, ctx, command):
        ...
        if command.description:
            ...
            self.description = command.help or "No help provided for now try it yourself"
    ...
    async def show_page(self, page, *, first=False):
        ...
        for entry in entries:
            self.embed.add_field(
                ...
                value=entry.short_doc or "No help provided for now try it yourself",
                inline=False,
            )
